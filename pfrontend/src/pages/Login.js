...
const Login = () => {
  ...
  const [messages, setMessages] = useState([
    { text: 'Hi! What is your name?', from: 'bot' },
  ]);
  ...
  const handleUserInput = async (value) => {
    ...
    switch (step) {
      case 'name':
        ...
        addMessage(`Nice to meet you, ${value.trim()}! What's your email?`, 'bot');
        setStep('email');
        break;
      ...
      case 'new_password':
        ...
        try {
          ...
          addMessage(res.data.message, 'bot');
          addMessage('You can now log in again. What is your name?', 'bot');
          setStep('name');
  ...
  const attemptLogin = async (data) => {
    ...
    try {
      ...
      addMessage('Or try again. What is your name?', 'bot');
      setStep('name');
      setFormData({ name: '', email: '', password: '' });
    }
  };
...
>
  Sign in with Google
        ...
        <Link style = {{alignItems:'center',justifyContent:'center',margin:'20px',color:'black'}} to="/register">Don't have an account?  Register Now...</Link>
      </div>
    </div>
  );
};
